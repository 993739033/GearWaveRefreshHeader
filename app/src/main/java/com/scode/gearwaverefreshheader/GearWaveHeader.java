...
///优化后的
public class GearWaveHeader extends View implements RefreshHeader {
    /**
     * 振幅
     */
    private float A;
    ...
    int animCircleTime = 800;//动画一周期时长
    ...
    public GearWaveHeader(Context context) {
        super(context);
        initView();
    }

    public GearWaveHeader(Context context, @Nullable AttributeSet attrs) {
        super(context, attrs);
        initView();
    }

    public GearWaveHeader(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
...
//        wavePaint.setColor(waveColor);
    ...
    private Path initGearPath() {
        Path path = new Path();
        insideR = (3 * Base_R + outsideRingWidthRatio * Base_R / 2);
        outsideR = (insideR + Base_R * gearWidthRatio);
        float temp = (-outsideR + offsetTemp);
...
//        LogUtil.d("offest:>>" + offset + "  height:>>" + height);
    ...
    private void startFinishAnim() {
        ValueAnimator animator = ValueAnimator.ofFloat(Base_R, Base_R * 1.2F, Base_R, Base_R * 0.6F);
    ...
    protected void onDetachedFromWindow() {
        ...
        if (waveAnimator != null) {
            waveAnimator.cancel();
            waveAnimator.addUpdateListener(null);
            waveAnimator = null;
        }
        if (gearAnimator != null) {
            gearAnimator.cancel();
            gearAnimator.addUpdateListener(null);
            gearAnimator.addListener(null);
            gearAnimator = null;
        }
        wavePath1 = null;
        wavePath2 = null;
